...
class warehouse:
    def __init__(self):
        self.data_warehouse = getattr(warehouse, "data_warehouse")
        self.model_warehouse = getattr(warehouse, "model_warehouse")
    ...
    def _set_model(self, model, model_id: str = None, storage: model_storages = model_storages.ram):
        ...
        actual_model_id = self.model_warehouse.set_model(model, model_id, storage)
        return actual_model_id

    def get_model(self, model_id: str):
        return self.model_warehouse.get_model(model_id)
    ...
    def _set_data(self, data, data_id: str = None, storage: data_storages = data_storages.ram):
        ...
        actual_data_id = self.data_warehouse.set_data(data, data_id, storage)
        return actual_data_id

    def get_data(self, data_id: str):
        return self.data_warehouse.get_data(data_id)

    def download_model(self, ftp_server_addr, server_file_name, user_name, password, data_id: str = None):
        return self.model_warehouse.download_model(ftp_server_addr, server_file_name, user_name, password, data_id)
